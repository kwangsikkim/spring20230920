...
import com.example.spring20230920.servise.MyService2;
...
public class Controller39 {
    ...
    @GetMapping("sub1")
    @ResponseBody  // ResponseEntity 면 생략 가능
...
//        return ResponseEntity.ok(data);
    ...
    @GetMapping("sub5")
    private ResponseEntity method5() {
        if (Math.random() > 0.5) {
            return ResponseEntity.ok().build(); // 2xx 번대 응답
        } else {
            return ResponseEntity.internalServerError().build(); // 5xx 번대 응답
        }
    }
